...
class RecruiterSearchFragment : BaseFragment() {
    ...
    private val sourcingCriteriaListTitleObject = SourcingCriteria(
        arrayListOf(),
        "",
        arrayListOf(),
        arrayListOf(),
        ...
        arrayListOf(),
        "",
        "ajouter un tag sourcing",
        arrayListOf(),
        "",
        0.0,
        0.0
    )
    private val sourcingCriteriaListAddMoreObject = SourcingCriteria(
        arrayListOf(),
        "",
        arrayListOf(),
        arrayListOf(),
    ...
    var sourcingCriteriaList =
        arrayListOf(sourcingCriteriaListTitleObject, sourcingCriteriaListAddMoreObject)
                ...
                when {
                    ...
                    result != null -> {
                        ...
                        if (mainObject.length() == 0){
                            BasicDialog(requireActivity(), "Aucun résultat. Modifiez vos critères de sourcing"){}.show()

                        }else{
                            for (i in 0 until mainObject.length()) {
                                sourcingSeekerList.add(
                                    Gson().fromJson(
                                        mainObject.getJSONObject(i).toString(),
                                        SourcingSeeker::class.java
                                    )
                                )
                            }
    ...
    private fun setSourcingCriteriaSpinnerAdapter() {
        spinnerAdapter = SourcingCriteriaSpinnerAdapter(
            requireActivity(),
            R.layout.item_textview,
            sourcingCriteriaList
        )
        ...
        spinnerAdapter.onAddSourcingClick = {
            RecruiterAddJobSourcingCriteriaDialogFragment().show(
                childFragmentManager,
                "RecruiterAddJobSourcingCriteriaDialogFragment"
            )
            binding.sp1.onDetachedFromWindow()
        }
    }
    ...
    private fun setSubscription() {
        ...
        binding.tvSubscription.text =
            "${companySubscription!!.usedSourcing}/${companySubscription!!.totalSourcings} available"
    }


    private fun getSourcingCriteria() {
        ParseCloud.callFunctionInBackground(
            ParseCloudFunction.GET_SOURCING_CRITERIA_V1.value,
            hashMapOf(
    ...
    private fun setCardStackView() {
        ...
        adapter.seeDescriptionClick = { sourcingSeeker ->
            ...
            binding.lJob.tvMatchedPer.text = "${sourcingSeeker.matchedPercentage}%"

            binding.lJob.tvJobLocation.text = sourcingSeeker.city.trim()
            try {
                val university = gson.fromJson(
                    JSONObject(sourcingSeeker.university).toString(),
            ...
            loadImageFromUrl(
                binding.lJob.circleImageView,
                sourcingSeeker.profileURL,
                R.drawable.default_user_img
            )

            val experiences = arrayListOf<Experience>()
            val experienceJsonArray = if (sourcingSeeker.experiences.isNotEmpty()){
            ...
            binding.lJob.cgMatches.removeAllViews()
            sourcingSeeker.matches.forEach {
                binding.lJob.cgMatches.addView(createChip(it, true))
            }
            binding.lJob.cgSoftSkillTags.removeAllViews()
            val softSkills = if (sourcingSeeker.softSkills.isNotEmpty()){
            ...
            val educationJsonArray = if (sourcingSeeker.educations.isNotEmpty()){
                JSONObject(sourcingSeeker.educations).getJSONArray("list")
            }else{
                JSONArray()
            }
            for (i in 0 until educationJsonArray.length()) {
                educations.add(
                    gson.fromJson(
                        educationJsonArray.getJSONObject(i).toString(),
            ...
            try {
                ...
                binding.lJob.rvHardSkill.adapter =
                    SeekerHardSkillAdapter(hardSkills, requireContext(), true)
            } catch (e: Exception) {
            }

            try {
                val volunteerings = arrayListOf<Volunteering>()
                val volunteeringJsonArray = if (sourcingSeeker.volunteerings.isNotEmpty()){
                    JSONObject(sourcingSeeker.volunteerings).getJSONArray("list")
                }else{
                    JSONArray()
                }
                 for (i in 0 until volunteeringJsonArray.length()) {
                    volunteerings.add(
                        gson.fromJson(
                            volunteeringJsonArray.getJSONObject(i).toString(),
                            Volunteering::class.java
                        )
                    )
                }
            ...
            binding.lJob.rvPortfolioLink.adapter = PortfolioLinksAdapter(sourcingSeeker.portfolioLinks as ArrayList<String>, true, requireContext(), showIcon = true, linkType = LinkType.Link.value).also {
                it.onLinkClick = { link ->
                    openLink(link)
                }
            }
            binding.lJob.rvPortfolioDocLink.adapter = PortfolioLinksAdapter(
                sourcingSeeker.portfolioDocLinks as ArrayList<String>,
                isStaticView = true,
                requireContext(),
                showIcon = true,
                LinkType.Doc.value
    ...
    private fun createChip(tag: String, isGradient: Boolean = false): View {
        val chip: View =
            layoutInflater.inflate(R.layout.item_chip_white_box_gradient_border, null, false)
    ...
    private fun setClickListeners() {
        ...
        binding.fabRefuse.setOnClickListener {
            val setting =
                SwipeAnimationSetting.Builder().setDirection(Direction.Left).setDuration(200)
        ...
        binding.fabShortlist.setOnClickListener {
            val setting =
                SwipeAnimationSetting.Builder().setDirection(Direction.Right).setDuration(200)
                    .setInterpolator(AccelerateInterpolator()).build()
            cardStackLayoutManager.setSwipeAnimationSetting(setting)
            binding.csvJob.swipe()
        }
        ...
        binding.ivFilterSourcing.setOnClickListener {
            RecruiterAddJobSourcingCriteriaDialogFragment.getInstance(currentSourcingId).show(childFragmentManager, "RecruiterAddJobSourcingCriteriaDialogFragment")
        }
    }
    ...
    private fun setCardStackListener() {
        cardStackListener = object : CardStackListener {
            ...
            override fun onCardSwiped(direction: Direction?) {
                if (currentSourcingId.isEmpty() || topJobSeekerId == null) {
                    return
                }
                when (direction) {
                    Direction.Left -> {
                    ...
                    Direction.Top -> {}
                    Direction.Right -> {
                        DialogAddToFolderOrOffer(
                            requireActivity(),
                            viewModel.jobTitleFilterList.value,
                            viewModel.seekerFolders.value,
                            { jobOffer ->
            ...
            override fun onCardRewound() {
                cardStackLayoutManager.topView.findViewById<ConstraintLayout>(R.id.cl_overlay).visibility =
                    View.GONE
                rewindJobSeekerId =
                    cardStackLayoutManager.topView.findViewById<TextView>(R.id.tv_job_seeker_id).text.toString()
                rewindSourcing()
            }
    ...
    private fun addOrRemoveFromSeekerFolder(
        type: Int,
        seekerFolder: SeekerFolder,
        isSuccess: (Boolean) -> Unit
    ) {
        if (type == 1) {
            swipedSourcingSeekerList.add(
                SwipedSourcingSeeker(
                    currentSourcingId,
                    topJobSeekerId!!,
                    true,
                    null,
        ...
        ParseCloud.callFunctionInBackground(
            ParseCloudFunction.HANDLE_FOLDER_ACTION_V1.value,
            request,
            FunctionCallback<Any> { result, e ->
                when {
                    e != null -> {
                    ...
                    result != null -> {
                        println("flkdsfklsd $request ${gson.toJson(result)}")
                        isSuccess(true)
                    }
                    else -> {
                        toast("Something Went Wrong.")
    ...
    private fun acceptOrUndoSourcing(
        type: Int,
        jobOffer: JobTitleFilter,
        isSuccess: (Boolean) -> Unit
    ) {
        if (type == 1) {
            swipedSourcingSeekerList.add(
        ...
        ParseCloud.callFunctionInBackground(
            ParseCloudFunction.SOURCING_ACCEPT_V1.value,
            request,
            FunctionCallback<Any> { result, e ->
                when {
                    e != null -> {
                    ...
                    result != null -> {
                        println("flkdsfklsd ${request[ParseTableFields.JOB_SEEKER_ID.value]} ${gson.toJson(result)}")
                        isSuccess(true)
                    }
                    else -> {
                        toast("Something Went Wrong.")
    ...
    private fun rejectOrUndoSourcing(type: Int, isSuccess: (Boolean) -> Unit) {
        ...
        ParseCloud.callFunctionInBackground(
            ParseCloudFunction.SOURCING_REJECT_V1.value,
            request,
            FunctionCallback<Any> { result, e ->
                when {
                    e != null -> {
...
class PortfolioImageAdapter2(val list: List<String>) :
    RecyclerView.Adapter<PortfolioImageAdapter2.ViewHolder>() {
    var onAddMoreClick: () -> Unit = {}
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder = ViewHolder(
        ItemPortfolioImage2Binding.inflate(
    ...
    class ViewHolder(val binding: ItemPortfolioImage2Binding) :
        RecyclerView.ViewHolder(binding.root)
}

class RecruiterSearchedJobCardStackAdapter(
    var list: ArrayList<SourcingSeeker>,
    val requireActivity: FragmentActivity
) : RecyclerView.Adapter<RecruiterSearchedJobCardStackAdapter.ViewHolder>() {
    var seeDescriptionClick: (SourcingSeeker) -> Unit = {}

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder = ViewHolder(
        ItemRecruiterJobCardBinding.inflate(
            LayoutInflater.from(parent.context),
            parent,
            false
        )
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.binding.apply {
            ...
            this.cgMatches.removeAllViews()
            sourcingSeeker.matches.forEach {
                this.cgMatches.addView(createChip(it))
            }

            this.tvMatchedPer.text = "${sourcingSeeker.matchedPercentage}%"
    ...
    class ViewHolder(val binding: ItemRecruiterJobCardBinding) :
        RecyclerView.ViewHolder(binding.root)

    private fun createChip(tag: String): View {
        val chip: View = requireActivity.layoutInflater.inflate(
            R.layout.item_chip_white_box_gradient_border,
            null,
            false
        )
        val tvContract = chip.findViewById<TextView>(R.id.tv_tag)
        tvContract.text = tag
        tvContract.background = requireActivity.resources.getDrawable(
            R.drawable.rounded_bottom_purple_top_blue_50,
            null
        )
        tvContract.updatePadding(30, 0, 30, 0)
...
class SourcingCriteriaSpinnerAdapter(
    private val activity: Activity,
    id: Int,
    var list: ArrayList<SourcingCriteria>
) : ArrayAdapter<SourcingCriteria>(activity, id, list) {
...
data class SwipedSourcingSeeker(
    val sourcingId: String,
    val jobSeekerId: String,
    val isSwipedRight: Boolean,
    val selectedJobOffer: JobTitleFilter?,
    val selectedSeekerFolder: SeekerFolder?
)
