...
class RecruiterAddJobSourcingCriteriaDialogFragment : DialogFragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        sourcingId = arguments?.getString("sourcing_id", "") ?: ""
    }
    ...
    private fun updateJobCriteriaList() {
        ...
        if (sourcingId.isNotEmpty()){
            val sc = sourcingCriteriaList.find { it.sourcingId == sourcingId }
            if (sc != null){
                val index = sourcingCriteriaList.indexOf(sc)
                binding.sp1.setSelection(index)
                spinnerAdapter.sourcingId = sourcingCriteriaList[index].sourcingId
            }else{
    ...
    private fun addSourcingCriteria() {

        if (binding.etTitle.text.isNullOrEmpty()){
            BasicDialog(requireActivity(), "Le titre de la balise ne peut être vide"){}.show()
            return
        }
    ...
    companion object {
        fun getInstance(sourcingId: String): RecruiterAddJobSourcingCriteriaDialogFragment {
            val fragment = RecruiterAddJobSourcingCriteriaDialogFragment()
            val bundle = Bundle()
            bundle.putString("sourcing_id", sourcingId)
