...
class PickVideoFragment : BaseFragment() {
   lateinit var binding: FragmentPickVideoBinding
    ...
    private fun setClickListeners() {
        binding.clVideo.setOnClickListener {
            if (checkForPermissions(arrayOf(Manifest.permission.CAMERA))){
                AddOrTakeVideoDialog(requireActivity(), onCamera = {
                    dispatchTakeVideoIntent()
                }, onGallery = {
                    dispatchAddVideoIntent()
                }).show()
            }
        }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == REQUEST_VIDEO_CAPTURE || requestCode == REQUEST_VIDEO_ADD && resultCode == Activity.RESULT_OK) {
            val videoUri: Uri? = data?.data
            if (videoUri != null){
                val durationInMillis = MediaMetadataRetriever().also { it.setDataSource(requireContext(), videoUri) }.extractMetadata(MediaMetadataRetriever.METADATA_KEY_DURATION)?.toLong() ?: 0
                if (durationInMillis < 31000){
                    ...
                    TitleMessageDialog(requireActivity(), "Longue vidéo !", "La vidéo doit être inférieure ou égale à 30 secondes.").show()
                }
             }
        }
    }
    ...
    private fun dispatchAddVideoIntent() {
