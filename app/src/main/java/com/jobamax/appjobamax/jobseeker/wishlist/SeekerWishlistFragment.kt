...
class SeekerWishlistFragment : BaseFragment() {
    ...
    private fun setWishlistAdapter() {
        ...
        object : SwipeHelper(requireContext(), binding.recyclerView, false) {
            override fun instantiateUnderlayButton(viewHolder: RecyclerView.ViewHolder?, underlayButtons: MutableList<UnderlayButton>?) {
                val archiveState = if(appliedFilter == WishlistJobFilter.ARCHIVE.value) "Unarchive" else "Archive"
                underlayButtons?.add(UnderlayButton(
                    archiveState,
                    AppCompatResources.getDrawable(requireContext(), R.drawable.ic_archive_white),
                    Color.parseColor("#2E68FF"),
                    Color.parseColor("#ffffff")
                ) { pos: Int ->
                    val item = wishlistAdapter.list[pos]
    ...
    private fun updateWishlist(filterId: Int) {
        when(filterId){
            ...
            WishlistJobFilter.TO_DO.value -> {
                wishlistAdapter.submitList(wishlist, WishlistJobFilter.TO_DO.value)
                wishlistAdapter.notifyDataSetChanged()
            }
...
//            }
    ...
    fun submitList(wishlist: ArrayList<WishlistJob>, filterId: Int) {
        list = when(filterId){
            ...
            SeekerWishlistFragment.WishlistJobFilter.TO_DO.value -> {
                wishlist.filter { !it.isApplied && !it.isArchived } as ArrayList
            }
