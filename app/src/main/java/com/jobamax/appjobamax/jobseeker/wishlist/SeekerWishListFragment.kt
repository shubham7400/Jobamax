...
import com.jobamax.appjobamax.databinding.ItemWishlistJobBinding
...
import com.jobamax.appjobamax.model.WishlistedJob
...
class SeekerWishListFragment : BaseFragmentMain<FragmentSeekerWishListBinding>() {
    ...
    private fun addToTrackingJobList(wishlistJob: WishlistedJob) {
        ...
        phase.no = "1"
        phase.date = SimpleDateFormat("MMM dd, yyyy", if (requireContext().getLanguage() == FRENCH_LANG_CODE){Locale.FRENCH}else{ Locale.US}).format(wishlistJob.pfObject?.createdAt)
            ...
            if (count == 0) {
                viewModel.addJobToTack(trackingJob.toParseObject()) {
                    if (it != null) {
                        ...
                        toast(getString(R.string.added_to_tracking_successfully))
                        fetchWishlist( )
                    }
                }
            } else {
