...
class ModifyCVFragment : BaseFragment(), ImagePicker.GetImage  {
    ...
    private fun setData()  {
        ...
        binding.tvJobLocation.text = "Localisation : "+jobSeeker.city
        ...
        // if education is empty then show the university name otherwise show the university of educations
        val university = gson.fromJson(jobSeeker.university, University::class.java)
        val universities = if (university != null) {
            arrayListOf(Education().also {
                it.name = university.name
                it.logo
            })
        }else{ arrayListOf() }
        university?.let { binding.cbUniversityStatus.isChecked = it.isCurrent }
        val customDropDownAdapter = CustomDropDownEducationAdapter(requireContext(), educations.ifEmpty { universities })
    ...
    private fun setPlaceAutocompleteFragment() {
        ...
        autocompleteFragment.setHint("Localisation")
    ...
    private fun setClickListeners() {
        ...
        binding.ivSetting.setOnClickListener {
            findNavController().navigate(R.id.action_modifyCVFragment_to_nav_seeker_account, null)
        }
        ...
        binding.ivAboutSkill.setOnClickListener {
        ...
        binding.llImportProfile.setOnClickListener { findNavController().navigate(R.id.action_modifyCVFragment_to_importProfileFromLinkedIn,  b)}
    ...
    private fun addSoftSkill() {
        ...
        if(binding.actvSoftSkill.text.isNullOrEmpty() || softSkill.isEmpty()){
            return
        }
    ...
    private fun addHardSkill() {
        ...
        if (binding.actvHardSkill.text.isNullOrEmpty() || hardSkill.isEmpty()){
            return
        }
        ...
        if (ownedHardSkills.filter { it.skillName == hardSkill }.isNotEmpty()) {
            ...
            ownedHardSkills.add(HardSkill(hardSkill, 50))
            hardSkillAdapter.submitList(ownedHardSkills)
            hardSkillAdapter.notifyDataSetChanged()
    ...
    private fun saveChanges() {
        ...
        viewModel.jobSeekerObject?.put(ParseTableFields.PROFILE_PERCENTAGE.value, calculateCompleteProfile(jobSeeker))
    ...
    override fun onDestroyView() {
        super.onDestroyView()
        (requireActivity() as SeekerCreateCvActivity).modifyCvScreenPositionY = binding.nestedScrollView.y.toInt()
    }

    override fun onResume() {
        super.onResume()
        if ((requireActivity() as SeekerCreateCvActivity).modifyCvScreenPositionY != 0){
            try {
                binding.nestedScrollView.scrollTo(0, (requireActivity() as SeekerCreateCvActivity).modifyCvScreenPositionY)
            }catch ( e: Exception){}
        }
    }
