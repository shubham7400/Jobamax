...
class MessagesAdapter @Inject constructor() :
    ...
    fun remoteAnItem(message: Message) {
        try {
    ...
    internal var deleteButtonClickListener: (Message) -> Boolean = { _ -> true }

    internal var shortListButtonClickListener: (Message) -> Boolean = { _ -> true }
    ...
    override fun onBindViewHolder(holder: Holder, position: Int) {
        ...
        if (name.trim().isEmpty()) {
            name = try {
                ...
                log("could not get the username of the receiver ${e.message}")
                ""
            }
        }
        ...
        holder.binding.apply {
            ...
            message.also { messageItem ->
                ...
                ivDelete.setOnClickListener {
                    ...
                    deleteButtonClickListener(messageItem)
                }

                // shortlist a message
                ivShortList.setOnClickListener {
                    slMessageSwipe.close(true)
                    shortListButtonClickListener(messageItem)
                }

                if (messageItem.shortListedBy.contains(currentUserId)) {
                    ivShortList.imageResource = R.drawable.ic_star_delete
                } else {
                    ivShortList.imageResource = R.drawable.ic_star
                }
    ...
    class Holder(val binding: ItemMessagesBinding) : RecyclerView.ViewHolder(binding.root) {
        companion object {
            fun from(parent: ViewGroup, layout: Int): Holder {
                val inflater = LayoutInflater.from(parent.context)
                val binding = DataBindingUtil.inflate<ItemMessagesBinding>(
                    inflater,
                    layout,
                    parent,
                    false
