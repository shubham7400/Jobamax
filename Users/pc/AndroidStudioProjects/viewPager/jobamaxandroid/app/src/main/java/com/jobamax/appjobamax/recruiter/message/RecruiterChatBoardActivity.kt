...
class RecruiterChatBoardActivity : BaseActivity() , ImagePicker.GetImage {
    lateinit var binding: ActivityRecruiterChatBoardBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityRecruiterChatBoardBinding.inflate(layoutInflater)
    ...
    override fun setImageUri(imageUri: Uri?, imageCode: Int) {
        imageUri?.let { uri ->
            lifecycleScope.launch(Dispatchers.IO){
                ...
                val compressedImage = Compressor.compress(this@RecruiterChatBoardActivity, file)
                val newUri = Uri.fromFile(compressedImage)
                when (imageCode) {
                    PROFILE_IMAGE_REQUEST_CODE -> {
                        uploadImageToParse(newUri, this@RecruiterChatBoardActivity,
                            { exception ->
                                toast(exception)
                            },
                            { url ->
