...
class TakeEmailFragment : Fragment() {
    ...
    private fun setClickListeners() {
        ...
        binding.etEmailName.addTextChangedListener {
            ...
            if (it != null) {
                if (!binding.cbCondition.isChecked || it.isEmpty() || binding.etEmailDomain.text.isEmpty()){
        ...
        binding.etEmailDomain.addTextChangedListener {
            user?.email = it.toString()
            if (it != null) {
                if (!binding.cbCondition.isChecked || binding.etEmailName.text.isEmpty() || it.isEmpty()){
                    binding.btnContinue.isEnabled = false
                    binding.btnContinue.background = resources.getDrawable(R.drawable.rounded_grey_box_50, null)
                    binding.btnContinue.setTextColor(resources.getColor(R.color.navi_blue, null))
                }else{
        ...
        binding.cbCondition.setOnCheckedChangeListener { _, isChecked ->
            if (!isChecked || binding.etEmailName.text.isEmpty() || binding.etEmailDomain.text.isEmpty()){
                binding.btnContinue.isEnabled = false
                binding.btnContinue.background = resources.getDrawable(R.drawable.rounded_grey_box_50, null)
                binding.btnContinue.setTextColor(resources.getColor(R.color.navi_blue, null))
            }else{
