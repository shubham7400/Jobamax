...
import com.jobamax.appjobamax.databinding.ActivitySeekerChatBoardBinding
...
class SeekerChatBoardActivity : AppCompatActivity() , ImagePicker.GetImage{
    lateinit var binding: ActivitySeekerChatBoardBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivitySeekerChatBoardBinding.inflate(layoutInflater)
    ...
    private fun initializeUi() {
        ...
        val recruiterPO = bundle.getParcelable<ParseObject>(ParseTableFields.RECRUITER.value)
    ...
    private fun setParseLiveQuery() {
        val parseQuery = ParseQuery<ParseObject>(ParseTableName.MESSAGE.value)
        parseQuery.whereEqualTo(ParseTableFields.THREAD_ID.value, chatThread!!.threadId)
        subscriptionHandling = MyParseLiveQueryClient.parseLiveQueryClient.subscribe(parseQuery)
        subscriptionHandling.handleSubscribe {
            subscriptionHandling.handleEvent(SubscriptionHandling.Event.CREATE) { _: ParseQuery<ParseObject?>?, result: ParseObject? ->
                result?.let {
                    ...
                    log("fksdkfjs ${newMessage.messageId}")
                    ...
                    if (!seenByList.contains( getUserId())){
    ...
    private fun setClickListeners() {
        ...
        binding.ivReport.setOnClickListener { ReportDialog(this){
            submitReport(it)
        }.show() }
    ...
    override fun setImageUri(imageUri: Uri?, imageCode: Int) {
        imageUri?.let { uri ->
            lifecycleScope.launch(Dispatchers.IO){
                ...
                val compressedImage = Compressor.compress(this@SeekerChatBoardActivity, file)
                val newUri = Uri.fromFile(compressedImage)
                when (imageCode) {
                    PROFILE_IMAGE_REQUEST_CODE -> {
                        uploadImageToParse(newUri, this@SeekerChatBoardActivity,
                            { exception ->
                                toast(exception)
                            },
                            { url ->
            ...
            TYPE_TEXT -> {
                (holder as TextViewHolder).binding.apply {
                    if (messageItem.senderId == requireContext.getUserId()) {
                        ...
                        this.tvMessageText.background = requireContext.resources.getDrawable(R.drawable.rounded_light_blue_box, null)
            ...
            TYPE_IMAGE -> {
                (holder as ImageViewHolder).binding.apply {
                    if (messageItem.senderId == requireContext.getUserId()) {
                        ...
                        this.llMessageImageBackground.background = requireContext.resources.getDrawable(R.drawable.rounded_light_blue_box, null)
            ...
            TYPE_AUDIO -> {
                (holder as AudioViewHolder).binding.apply {
                    if (messageItem.senderId == requireContext.getUserId()) {
                        ...
                        this@apply.clAudioPlayer.background = requireContext.resources.getDrawable(R.drawable.rounded_light_blue_box, null)
                    ...
                    this@apply.ivAudioPlayBtn.setOnClickListener {
                        if (player.isPlaying) {
                            player.pause()
                            this@apply.ivAudioPlayBtn.setImageResource(R.drawable.play)
                        }else{
                            player.start()
                            this@apply.ivAudioPlayBtn.setImageResource(R.drawable.pause)
